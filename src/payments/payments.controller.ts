...
export class PaymentsController {
    ...
    async initiatePayment(@Body() body: { userId: number; email: string; plan?: string }) {
    ...
    async callback(
        ...
        try {
            if (status !== 'OK') {
                this.logger.warn(`Payment failed or canceled. Authority: ${authority}, Status: ${status}`);
                return res.redirect(`${FRONTEND_URL}/payment/failed?reason=canceled`);
            }

            const amount = 199000;
            const verification = await this.zarinpalService.verifyPayment(authority, amount);
            ...
            return res.redirect(`${FRONTEND_URL}/payment/success?authority=${authority}&refId=${verification.refId}`);
        } catch (error: any) {
    ...
    async activateAccount(
        ...
        try {
            ...
            this.logger.log('✅ Payment verified:', { authority, refId: verification.refId });
            ...
            if (existingUser) {
                ...
                await this.db
                    .update(users)
                    .set({ active: true, subscriptionExpiresAt, verified: true, isPremium: true })
                    .where(eq(users.email, email));

                this.logger.log(`✅ Existing user ${email} reactivated. Expires at: ${subscriptionExpiresAt}`);
            ...
            const [pendingUser] = await this.db.select().from(pending_users).where(eq(pending_users.email, email));

            if (!pendingUser) {
                this.logger.error('No pending user found:', email);
                return res.status(HttpStatus.NOT_FOUND).send({ error: 'Pending user not found' });
            }

            if (!pendingUser.verified) {
                this.logger.error('Pending user email not verified:', email);
                return res.status(HttpStatus.BAD_REQUEST).send({ error: 'Email not verified' });
            }
