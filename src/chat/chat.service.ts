...
const IMAGE_KEYWORDS = [
  ...
  'fotoğraf oluştur',
  'fotoğraf çiz',
  // Hayvan/nesne + resim kombinasyonları
  'resmi yap',
  'resmini yap',
  'resmini çiz',
  'resmi çiz',
...
export class ChatService {
  ...
  private isImageRequest(message: any): boolean {
    ...
    if (typeof message === 'string') {
      ...
      const isImage = IMAGE_KEYWORDS.some((keyword) => lowerMessage.includes(keyword));
      this.logger.log(`[isImageRequest] Checking: "${lowerMessage.substring(0, 50)}" => ${isImage}`);
      return isImage;
    }
  ...
  private async extractPdfText(base64Data: string): Promise<string> {
    try {
      ...
      this.logger.log(
        `[PDF] Extracted ${data.text.length} characters from PDF`,
      );
      return data.text;
    } catch (error) {
  ...
  async createSession(userId: number, title?: string) {
    ...
    if (verification.length === 0) {
      console.error(
        `[ChatService] ❌ Session ${session.id} not found after creation!`,
      );
    } else {
      console.log(
        `[ChatService] ✅ Session ${session.id} verified in database`,
      );
    }
  ...
  async sendMessage(
    ...
    if (!actualSessionId) {
      const session = await this.createSession(userId, 'New Chat');
      actualSessionId = session.id;
    } else {
      await this.ensureSessionOwnership(actualSessionId, userId);
    }
    ...
    this.logger.log(
      `[sendMessage] Processed message type: ${typeof processedMessage}, isArray: ${Array.isArray(processedMessage)}`,
    );

    // Serialize array messages as JSON for database storage
    const contentToStore = Array.isArray(processedMessage)
    ...
    const messageCount = await this.db
      .select()
      .from(messages)
      .where(eq(messages.sessionId, actualSessionId));
    ...
    if (messageCount.length === 1) {
      ...
      if (titleText) {
        ...
        await this.db
          ...
          .where(eq(sessions.id, actualSessionId));
      }
    }
    ...
    if (mode === 'image' || this.isImageRequest(processedMessage)) {
      ...
      return { response: imageResponse, userMessageId: userMsg.id, sessionId: actualSessionId };
    }
    ...
    if (mode === 'web') {
      ...
      if (bingKey) {
        ...
        const [assistantMsg] = await this.db
          ...
          .returning();
        await this.touchSession(actualSessionId);
        return {
      ...
      const history = await this.getRecentMessages(actualSessionId);
  ...
  async sendMessageStream(
    ...
    const processedMessage = await this.processMessageContent(userMessage);
    this.logger.log(
      `[sendMessageStream] Processed message type: ${typeof processedMessage}, isArray: ${Array.isArray(processedMessage)}`,
    );
...
Örnek: "Güncel haberler için [TRT Haber](https://www.trthaber.com) sitesini ziyaret edebilirsiniz."`;
  ...
  private async ensureSessionOwnership(sessionId: string, userId: number) {
    ...
    if (isVerbose) {
      console.log(
        `[ensureSessionOwnership] Checking session ${sessionId} for user ${userId}...`,
      );
    }
    ...
    if (!session) {
      console.error(
        `[ensureSessionOwnership] ❌ Session ${sessionId} not found in database`,
      );
    ...
    if (isVerbose) {
      console.log(
        `[ensureSessionOwnership] Found session ${sessionId}, owner: ${session.ownerId}, requesting user: ${userId}`,
      );
    }
    ...
    if (normalizedOwnerId !== normalizedUserId) {
      console.error(
        `[ensureSessionOwnership] ❌ User ${normalizedUserId} doesn't own session ${sessionId} (owner: ${normalizedOwnerId})`,
      );
      throw new Error('Session not found or unauthorized');
    }
